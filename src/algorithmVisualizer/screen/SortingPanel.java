...
import algorithmVisualizer.customComponent.CustomSwingWorker;
import algorithmVisualizer.customComponent.CustomPanel;


public class SortingPanel extends CustomPanel {
	...
	private void initBarsDimension() {
		bar_width = (double) CUSTOM_PANEL_WIDTH / size;
		bar_height = new double[size];
		
		double height_interval = (double) CUSTOM_PANEL_HEIGHT /size;
	...
	public void paintComponent(Graphics g) {
		...
		for(int i = 0; i < size; i++) {
			x = i * bar_width;
			y = CUSTOM_PANEL_HEIGHT - (BAR_STROKE_THICKNESS + bar_height[i]);
	...
	private void drawAlgorithm(Graphics2D g2d) {
		...
		if((current_bar_index = current_algorithm.get_current_index()) != -1) {
			...
			y = CUSTOM_PANEL_HEIGHT - (BAR_STROKE_THICKNESS + bar_height[current_bar_index]);
		...
		if((traversing_bar_index = current_algorithm.get_traversing_index()) != -1) {
			...
			y = CUSTOM_PANEL_HEIGHT - (BAR_STROKE_THICKNESS + bar_height[traversing_bar_index]);
		...
		if((selected_bar_index = current_algorithm.get_selected_index()) != -1) {
			...
			y = CUSTOM_PANEL_HEIGHT - (BAR_STROKE_THICKNESS + bar_height[selected_bar_index]);
		...
		int string_y_padding = 20, string_x_padding = 200, string_tab = 20;
