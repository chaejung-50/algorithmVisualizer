...
import algorithmVisualizer.algorithms.SortingAlgorithm;

public class SidePanel extends CustomPanel{
	...
	private void initComponent(int width, int height) {
		...
		int y = 0, font_size = 30;
		
		String title_text = "<html><center>SORTING<br/>ALGORITHM</center></html>";
		add(new CustomLabel(new Rectangle(fixed_x, y, width, title_height), title_text, new Font("Arial", Font.PLAIN, font_size)));
		
		font_size = 20;
		y = title_height;
		int selected_algo_height = title_height;
		String selected_algorithm_text = "NONE";
		CustomLabel selected_algorithm = new CustomLabel(new Rectangle(fixed_x, y, width, selected_algo_height),
																	   selected_algorithm_text, new Font("Arial", Font.PLAIN, font_size)) {
			private static final long serialVersionUID = 1L;
		...
		AbstractAction action = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				String text = ((CustomButton)e.getSource()).getText();
		...
		int button_height = (height - (title_height + selected_algo_height)) / (SortingAlgorithm.values().length - 1);
		...
		for(SortingAlgorithm algo : SortingAlgorithm.values()) {
			...
			add(new CustomButton(new Rectangle(fixed_x, y, width, button_height),
											   action, algo.toString().replace("_", " ")));
			y += button_height;
		}
		...
		side_panel_animation1 = new SwingWorker<>(){
			...
			protected Void doInBackground() throws InterruptedException {
				while(!isCancelled()) {
					...
					else if(!is_showing_in && getX() >= -1 * CUSTOM_PANEL_WIDTH)
						setLocation(getX() - 1, getY());
					else
						break;
				}
